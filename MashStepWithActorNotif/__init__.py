...
class NotificationStepActorNotif(CBPiStep):
    ...
    async def on_start(self):
        # We start the actor identified in the property
    ...
    async def on_stop(self):
        # We stop the actor identified in the property
...
@parameters([Property.Number(label="Temp", configurable=True),
             ...
             Property.Actor(label="Actor", description="Actor To set On/off during step (step indicator for path"),
...
class MashInStepActorNotif(CBPiStep):
...
@parameters([Property.Number(label="Timer", description="Time in Minutes", configurable=True), 
             ...
             Property.Kettle(label="Kettle"),
             Property.Actor(label="Actor", description="Actor To set On/off during step (step indicator for path"),
             Property.Text(label="Notification",configurable = True, description = "Text for notification when timer is finnished"),
             Property.Select(label="AutoMode",options=["Yes","No"], description="Switch Kettlelogic automatically on and off -> Yes")])
class MashStepActorNotif(CBPiStep):
...
@parameters([Property.Number(label="Timer", description="Time in Minutes", configurable=True),
             Property.Actor(label="Actor", description="Actor To set On/off during step (step indicator for path")])
class WaitStepActorNotif(CBPiStep):
...
class ToggleStepActorNotif(CBPiStep):
    async def on_timer_done(self, timer):
        self.summary = ""
        await self.next()
...
@parameters([Property.Number(label="Timer", description="Time in Minutes", configurable=True),
             Property.Actor(label="Actor",description="Actor 1"),
             Property.Actor(label="Actor2",description="Actor2")])
class ActorStepActorNotif(CBPiStep):
    async def on_timer_done(self, timer):
        self.summary = ""
        await self.next()
    ...
    async def on_start(self):
        # We start the actor identified in the property
        await self.actor_on(self.props.Actor2)
        if self.timer is None:
            self.timer = Timer(int(self.props.Timer) * 60, on_update=self.on_timer_update, on_done=self.on_timer_done)
        self.timer.start()
        await self.actor_on(self.props.Actor)
    ...
    async def on_stop(self):
        # We stop the actor identified in the property
        await self.actor_off(self.props.Actor2)
        await self.actor_off(self.props.Actor)
        await self.timer.stop()
        self.summary = ""
        await self.push_update()
...
@parameters([Property.Number(label="Timer", description="Time in Minutes", configurable=True), 
             ...
             Property.Number("TimeIndicatorInStep", configurable=True, description="Timer for optionnal notification (minutes before finish"),
             Property.Text(label="OptionnalNotification",configurable = True, description = "Text for optionnal notification"),
             Property.Text(label="NotificationEnd",configurable = True, description = "Text for notification when boil is finished")])
class BoilStepActorNotif(CBPiStep):
    ...
    async def on_timer_done(self,timer):
        ...
        if self.AutoMode == True:
            await self.setAutoMode(False)
        self.cbpi.notify(self.name, self.props.get("NotificationEnd",""), NotificationType.SUCCESS)
        await self.next()
    ...
    async def check_optionnalNotif_timer(number):
            if self.remaining_seconds != None and self.remaining_seconds <= (int(value) * 60 + 1):
                self.cbpi.notify(self.name, self.props.get("OptionnalNotification",""), NotificationType.INFO)
    ...
    async def run(self):
        ...
        while self.running == True:
            ...
            await self.check_optionnalNotif_timer(self.props.get("OptionnalNotification", None))
...
@parameters([Property.Number(label="Temp", configurable=True, description="Target temperature for cooldown. Notification will be send when temp is reached and Actor can be triggered"),
             ...
             Property.Actor(label="Actor2", description="Actor to trigger for monitoring steps"),
             Property.Kettle(label="Kettle")])
class CooldownStepActorNotif(CBPiStep):
...
def setup(cbpi):
    ...
    cbpi.plugin.register("MashInStepActorNotif", MashInStepActorNotif)
    cbpi.plugin.register("MashStepActorNotif", MashStepActorNotif)
    cbpi.plugin.register("BoilStepActorNotif", BoilStepActorNotif)
    cbpi.plugin.register("CooldownStepActorNotif", CooldownStepActorNotif)
    cbpi.plugin.register("WaitStepActorNotif", WaitStepActorNotif)
    cbpi.plugin.register("ToggleStepActorNotif", ToggleStepActorNotif)
    cbpi.plugin.register("ActorStepActorNotif", ActorStepActorNotif)
    cbpi.plugin.register("NotificationStepActorNotif", NotificationStepActorNotif)